...
export default function BasketSharePage() {
  ...
  if (!basket) {
    return (
      <div style={{ minHeight: "100vh", background: "#080808", color: "#ede9e3" }}>
        <Header />
        <GridOverlay />
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 10, maxWidth: 960, margin: "0 auto", padding: "110px 24px 80px" }}>
        ...
        <h1 style={{ color: "#ede9e3", fontSize: "clamp(34px, 5vw, 56px)", lineHeight: 1.02, letterSpacing: "-0.05em", margin: "0 0 10px" }}>
          {basket.title}
        </h1>
        <p style={{ color: "#988f87", fontSize: 17, lineHeight: 1.65, margin: "0 0 26px", maxWidth: 780 }}>
          {basket.summary}
        </p>
        ...
        <section style={sectionStyle}>
          ...
          <div style={{ color: "#ede9e3", fontSize: 24, fontWeight: 600, marginBottom: 10 }}>{beliefSummary?.core_belief}</div>
          <div style={{ color: "#928981", lineHeight: 1.7 }}>
            <strong style={{ color: "#ddd4cc" }}>Resolution target:</strong> {beliefSummary?.resolution_target}<br />
            <strong style={{ color: "#ddd4cc" }}>Mechanism:</strong> {beliefSummary?.mechanism}
          </div>
        </section>
        ...
        {(parsedBasket as PredictionBasket).holdings.map((holding) => (
              <div key={holding.ticker} style={{ border: "1px solid rgba(255,255,255,0.08)", borderRadius: 18, padding: 18, background: "rgba(255,255,255,0.02)" }}>
        ...
        {analysis?.most_surprising_connection && (
          <section style={sectionStyle}>
            ...
            <div style={{ color: "#978f87", lineHeight: 1.7 }}>{analysis.most_surprising_connection}</div>
          </section>
        )}
